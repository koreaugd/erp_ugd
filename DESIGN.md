...
- **헤더(thead th / 세로형이면 왼쪽 라벨 열)**: 배경 `--branch-vanilla`(#EFF0A3) + **1px solid 검정 테두리**(위·오른·아래, 첫 칸 왼쪽). **헤더 글자는 반드시 볼드(`font-black`, 900) · 12px(text-xs) · 솔리드 검정** — 근무자·급여대장 헤더와 똑같이. **주의: 글자색에 `text-gray-*`를 쓰지 말 것** — `.branch-redesign`가 68% 회색으로 치환해 볼드가 흐려 보인다. `text-[#212121]`(또는 `--branch-black`)로 100% 검정을 준다. (본문 값 칸은 볼드 아님/옅은 격자로 헤더와 대비를 준다.)
