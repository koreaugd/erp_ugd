...
- **헤더(thead th)**: 배경 `--branch-vanilla`(#EFF0A3) + **1px solid 검정 테두리**(위·오른·아래, 첫 칸 왼쪽). 글자 검정·`font-black`·가운데 정렬.
- **셀 입력칸**: `sheet-cell-input` 클래스로 **납작 투명**(배경·테두리·라운드 없음). 둥근 입력박스 금지. 셀 높이는 `h-9`로 통일.
- **격자선**: 감싸는 `<td>`가 `border-r border-b border-black/10`(첫 칸 `border-l`)로 옅게 긋는다. `table`은 `border-separate; border-spacing:0`.
...
- **컬럼 폭**: `w-full` 표에서 컬럼 수가 적으면 각 칸이 남는 폭을 나눠 가져 **과도하게 넓어진다.** 데이터 칸엔 `w-20`~`w-28` 등 고정폭을 주고, **긴 텍스트 칸(사유 등) 하나만 폭 없이 두어 남는 폭을 흡수**시킨다.
