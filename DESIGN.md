...
- 이제 세부 항목(전일 정산현황·변경이력 & 수기대장 / 제출현황·현금관리·현금지출)을 각 그룹(`일일업무`·`월말업무`) 아래 **`.ugd-nav-item`으로 바로 노출**한다. 클릭하면 해당 `adminSection`과 세부 탭을 함께 설정하고, 활성 표시는 다른 메뉴 항목과 동일한 바닐라(`#EFF0A3`)다.
