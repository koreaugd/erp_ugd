...
지점 화면과 관리자 화면의 사이드바는 **같은 형태**를 쓴다. 외부 레퍼런스(셀러닷컴)의 밝은 배경 + 섹션 그룹 + 촘촘한 목록을 **거의 그대로** 따르되, **활성(현재 탭) 표시색만 ERP 바닐라**로 바꾼다. 공통 클래스: `.ugd-nav-group`(그룹 헤더), `.ugd-nav-item`(메뉴 항목), `.ugd-subnav-item`(관리자 하위탭).
...
**그룹 헤더 (`.ugd-nav-group`) — 회색 라벨(채우지 않음)**
- 레퍼런스의 `지도/쇼핑/도구`처럼 **회색 텍스트만(볼드)**: `11px / 800`, 자간 0.33px, `rgba(33,33,33,0.55)`, `padding: 14px 20px 6px`, **배경 없음**.
- 지점: `메인 / 일일 업무 / 월말 업무 / 인사·기타`. 관리자: `정산 / 인사 / 이동`.
- 모바일(≤767px)에서는 숨김.
...
**관리자 하위탭 (`.ugd-subnav-item`)**
- 일일정산/월말마감을 눌렀을 때 아래 펼쳐지는 하위 항목. `12px / 500`, `padding: 6px 12px`, 활성은 **바닐라 `#EFF0A3`**. `ml-4 pl-3 border-l`로 한 단계 들여쓴다.
