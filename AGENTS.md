...
- **급여대장을 건드리면 해당 규칙 검증을 돌린다.** 서버 접속 없이 도는 읽기 전용 스크립트이고, 과거에 실제로
  터졌던 사고(한 사람이 두 줄로 나가 급여 이중지급 · 남의 계좌·주민등록번호가 복사됨 · 저장 경쟁으로 옛 값이
  최종본으로 남음)를 규칙으로 못 박아 둔 것이라 **눈으로는 잡히지 않는다.** 실패하면 배포하지 않는다.
  - 정직원: `npx tsx scripts/check-fulltime-salary.ts` (행 병합 · 전월 이어받기 · 순서 · 자동저장 직렬화 · 보안 게이트)
  - 파트타이머: `npx tsx scripts/check-parttime-dedupe.ts` (유령 행 흡수 · 중복 방지)
  - 두 스크립트는 순수 함수 검증 + **화면 소스에 방어가 남아 있는지 훑는 검사**를 함께 한다. 방어를 떼면
    실제로 실패하는 것까지 확인해 두었으니, 검사가 걸리면 "검사가 낡았다"고 보지 말고 **코드를 먼저 의심할 것.**
